...
- 未完成任务默认保留：扫描时对 TODO/FIXME、阶段/任务编号标记、任务计划等做完成情况检查（complete / incomplete / unknown，unknown 按 incomplete 处理）；未完成/无法判定的任务默认**保持未完成状态**，完整保留其上下文（意图与完成定义、剩余工作、约束、续作位置、关联产物），并在计划（`deferred_tasks`）与报告中显式标记为"未处理"——是有意保留，不是遗漏。默认不扫描代码核对完成情况，也不处理未完成任务；仅在调用时显式要求时才切换，全程无需额外人工确认。
- 统一前置问答门：唯一交互点在 INIT（baseline 捕获后），一次性确认清理范围（全仓库 / 仅未提交 / 最近提交 / 指定路径 / 仅审计）与 L3 授权；之后全程无人值守，未决事项运行末 escalation。无法交互时默认仅未提交改动并记录验证缺口。未完成任务策略不进入问答门：默认保留并标记未处理，仅当调用时显式要求才切换为"扫描代码核对完成情况"或"处理未完成任务"。
...
未完成任务默认保留：TODO/FIXME、阶段/任务编号标记、任务计划会被检查完成情况，未完成/无法判定的一律保持原样、完整保留上下文并标记为"未处理"，不需要任何额外确认。需要改变默认行为时，在调用里显式说明：

```text
使用 RepoMeld 清理当前仓库，并扫描代码核对 TODO 的完成情况；只处理已证实完成的，未完成的保留并在报告中列出。
```

```text
使用 RepoMeld 清理当前仓库，未完成的 TODO 也一并按范本规范化处理。
```
...
│   ├── risk-policy.md
│   ├── task-completion-policy.md  # 任务完成情况检查与未完成任务默认保留策略
